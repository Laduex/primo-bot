...
class OpsAlertService:
    ...
    async def send_alert(self, request: OpsAlertRequest) -> OpsAlertResult:
        ...
        if not self._target_channel_id.isdigit():
            return OpsAlertResult(False, "OPS_ALERT_CHANNEL_ID is invalid")
        ...
        alert = format_alert(request)
        for index, chunk in enumerate(chunk_text(alert.message, EMBED_DESCRIPTION_LIMIT), start=1):
...
def format_alert(request: OpsAlertRequest) -> AlertView:
    severity = normalize_severity(request.severity)
    title = clean(request.title, "Production alert")
    message = format_message(clean(request.message, "(no details)"))
...
def normalize_severity(severity: str | None) -> str:
    value = clean(severity, "info").lower()
...
def color_for(severity: str) -> int:
